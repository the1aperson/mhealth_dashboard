...
/**
 * This is the model class for table "signature_data".
 *
 * @property int $id
 ...
 * @property ParticipantTestSession $testSession
 */
class SignatureData extends \yii\db\ActiveRecord
{
	const RAW_TYPE_JPEG = 10;
	const RAW_TYPE_PNG = 20;
	const RAW_TYPE_OTHER = 30;
    ...
    {
        return [
            [['participant', 'device', 'raw_type', 'created_at'], 'integer'],
            [['blob_data'], 'string'],
    ...
    {
	    if(stristr($mimeType, "jpg") || stristr($mimeType, "jpeg"))
	    {
		    return SignatureData::RAW_TYPE_JPEG;
	    }
	    
	    if(stristr($mimeType, "png"))
	    {
		    return SignatureData::RAW_TYPE_PNG;
	    }
	    
	    return SignatureData::RAW_TYPE_OTHER;
    }

    public static function createSignatureData($participant_id, $device_id, $file)
    {
	    $fh = fopen($file->tempName, 'rb');
		$image_contents = fread($fh, $file->size);
		fclose($fh);
		
	    $newSigData = new SignatureData();
	    $newSigData->participant = $participant_id;
	    $newSigData->device = $device_id;
	    $newSigData->blob_data = $image_contents;
	    $newSigData->raw_type = SignatureData::getTypeFromMimeType($file->type);
	    $newSigData->save();
	    return $newSigData;
    }
